"""Audit V4.8 fresh calibration collection before human review."""
...
def main() -> None:
    ...
    workbook_path = (
        root / "benchmarks/verification_construction_v4_stage8_fresh_calibration_workbook_v1.json"
    )
    workbook = load_replacement_calibration_workbook(workbook_path)
    retired_claims: set[str] = set()
    retired_domains: set[str] = set()
    for path in root.glob("benchmarks/verification_construction_v3*.json"):
    ...
    audit = {
        "audit_id": "verification-construction-v4-stage8-collection-audit-v1",
    ...
    destination = (
        root / "artifacts/evaluations/verification-construction-v4-stage8-collection-audit-v1.json"
    )
