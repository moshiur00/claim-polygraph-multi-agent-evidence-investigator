"""Audit V3.6c fresh integrity and decide whether held-out may open."""
...
def main() -> None:
    ...
    if result["calibration_executions"] != 1:
        raise ValueError("fresh calibration execution count is not one")
    if (
        result["original_held_out_cases_loaded"]
        or result["original_held_out_cases_exposed_to_model"]
    ):
        raise ValueError("original held-out data crossed the V3.6c boundary")
    failed = [
    ...
    audit = {
        "audit_id": (
            "verification-construction-v3-stage6c-fresh-calibration-audit-v1"
        ),
        ...
        "decision": (
            "The fresh calibration passed every frozen gate. The original "
