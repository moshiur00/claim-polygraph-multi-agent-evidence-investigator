"""Execute the approved V3.6c fresh calibration exactly once."""
...
MODEL = "gpt-5.6-luna"
EXPERIMENT_ID = uuid5(
    NAMESPACE_URL, "claim-polygraph/v3.6c/replacement-calibration/v1"
)


def _verify_freeze(root: Path, manifest: dict) -> None:
    if manifest["status"] != "frozen" or manifest["execution_limit"] != 1:
        raise ValueError("V3.6c fresh manifest is not a one-run freeze")
...
async def main() -> None:
    ...
    ledger = SQLitePaidOperationLedger(root / "data/v3-stage6c-paid-operations.db")
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V3.6c paid-operation ledger is not empty before first run")
    telemetry = TelemetryCollector(root / "data/v3-stage6c-telemetry.db")
    ...
    for case in workbook.cases:
        ...
        if eligibility not in callable_eligibility:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v3.6c/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL,
                    f"claim-polygraph/v3.6c/{case.case_id}/{item.evidence_id}",
                ),
                claim_id=claim_id,
        ...
        wrapped = IdempotentStructuredModelProvider(
            ...
            worker_id="v3.6c-replacement-calibration",
        )
    ...
    artifact = {
        "evaluation_id": (
            "verification-construction-v3-stage6c-fresh-calibration-v1"
        ),
        "status": (
