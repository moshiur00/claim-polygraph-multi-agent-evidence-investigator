"""Execute the approved V3.6a replacement calibration exactly once."""
...
MODEL = "gpt-5.6-luna"
EXPERIMENT_ID = uuid5(
    NAMESPACE_URL, "claim-polygraph/v3.6a/replacement-calibration/v1"
)


def _verify_freeze(root: Path, manifest: dict) -> None:
    if manifest["status"] != "frozen" or manifest["execution_limit"] != 1:
        raise ValueError("V3.6a replacement manifest is not a one-run freeze")
...
async def main() -> None:
    ...
    if result_path.exists():
        raise FileExistsError("replacement calibration has already been executed")
    freeze_path = (
        root
        / "artifacts/evaluations/"
    ...
    ledger = SQLitePaidOperationLedger(root / "data/v3-stage6a-paid-operations.db")
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V3.6a paid-operation ledger is not empty before first run")
    telemetry = TelemetryCollector(root / "data/v3-stage6a-telemetry.db")
    ...
    for case in workbook.cases:
        ...
        if eligibility not in callable_eligibility:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v3.6a/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL,
                    f"claim-polygraph/v3.6a/{case.case_id}/{item.evidence_id}",
                ),
                claim_id=claim_id,
        ...
        wrapped = IdempotentStructuredModelProvider(
            ...
            worker_id="v3.6a-replacement-calibration",
        )
    ...
    artifact = {
        "evaluation_id": (
            "verification-construction-v3-stage6a-replacement-calibration-v1"
        ),
        "status": (
        ...
        "replacement_calibration_cases_exposed_to_model": sum(
