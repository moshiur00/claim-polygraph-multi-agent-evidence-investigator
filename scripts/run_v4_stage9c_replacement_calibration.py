"""Run the frozen, receipt-protected V4.9c calibration exactly once."""
...
EXPERIMENT_ID = uuid5(NAMESPACE_URL, "claim-polygraph/v4.9c/replacement-calibration/v1")
MODEL = "gpt-4o-mini"
MAXIMUM_CALLS = 20
CALLABLE = {
...
async def main() -> None:
    ...
    destination = (
        evaluations / "verification-construction-v4-stage9c-replacement-calibration-v1.json"
    )
    if destination.exists():
        raise FileExistsError("V4.9c calibration has already been executed")
    ...
    if freeze["status"] != "frozen" or freeze["execution_limit"] != 1:
        raise ValueError("V4.9c is not protected by a one-execution freeze")
    ...
    ledger = SQLitePaidOperationLedger(root / "data/v4-stage9c-paid-operations.db")
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V4.9c receipt identity is not empty; refusing a rerun")
    telemetry = TelemetryCollector(root / "data/v4-stage9c-telemetry.db")
    ...
    for case in cases:
        ...
        if eligibility not in CALLABLE:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v4.9c/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL,
                    f"claim-polygraph/v4.9c/{case.case_id}/{item.evidence_id}",
                ),
                claim_id=claim_id,
        ...
        wrapped = IdempotentStructuredModelProvider(
            ...
            worker_id="v4.9c-replacement-calibration",
        )
    ...
    artifact = {
        "evaluation_id": "verification-construction-v4-stage9c-replacement-calibration-v1",
        "status": (
