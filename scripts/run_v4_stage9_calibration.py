"""Run the frozen, receipt-protected V4.9 calibration exactly once."""
...
EXPERIMENT_ID = uuid5(NAMESPACE_URL, "claim-polygraph/v4.9/calibration/v1")
MODEL = "gpt-4o-mini"
MAXIMUM_CALLS = 20
CALLABLE = {
...
async def main() -> None:
    ...
    destination = evaluations / "verification-construction-v4-stage9-calibration-v1.json"
    if destination.exists():
        raise FileExistsError("V4.9 calibration has already been executed")

    freeze_path = evaluations / "verification-construction-v4-stage9-calibration-freeze-v1.json"
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    if freeze["status"] != "frozen" or freeze["execution_limit"] != 1:
        raise ValueError("V4.9 is not protected by a one-execution freeze")
    ...
    ledger = SQLitePaidOperationLedger(root / "data/v4-stage9-paid-operations.db")
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V4.9 receipt identity is not empty; refusing a rerun")
    telemetry = TelemetryCollector(root / "data/v4-stage9-telemetry.db")
    ...
    for case in cases:
        ...
        if eligibility not in CALLABLE:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v4.9/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL, f"claim-polygraph/v4.9/{case.case_id}/{item.evidence_id}"
                ),
                claim_id=claim_id,
    ...
    artifact = {
        "evaluation_id": "verification-construction-v4-stage9-calibration-v1",
        "status": (
