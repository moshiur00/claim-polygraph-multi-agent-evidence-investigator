"""Execute the frozen V3.7 held-out split exactly once."""
...
from run_v3_stage6e_fresh_calibration import _kind
...
def _verify_freeze(root: Path, manifest: dict) -> None:
    if manifest["status"] != "frozen" or manifest["execution_limit"] != 1:
        raise ValueError("V3.7 manifest is not a one-run freeze")
    if manifest["configuration_changed_after_calibration"]:
        raise ValueError("held-out configuration differs from passing calibration")
...
async def main() -> None:
    ...
    if result_path.exists():
        raise FileExistsError("V3.7 held-out evaluation has already been executed")
    freeze_path = (
        root
        / "artifacts/evaluations/"
    ...
    if [case.case_id for case in cases] != manifest["case_ids"]:
        raise ValueError("held-out selection differs from frozen manifest")

    deterministic = {case.case_id: _evaluate_case(case) for case in cases}
    ...
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V3.7 paid-operation ledger is not empty before first run")
    telemetry = TelemetryCollector(root / "data/v3-stage7-telemetry.db")
    ...
    for case in cases:
        ...
        if eligibility not in callable_eligibility:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v3.7/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL,
                    f"claim-polygraph/v3.7/{case.case_id}/{item.evidence_id}",
                ),
                claim_id=claim_id,
    ...
    for item in results:
        annotation = cases_by_id[item["case_id"]].annotation
    ...
    artifact = {
        "evaluation_id": "verification-construction-v3-stage7-held-out-v1",
        "status": (
            "completed_passed" if all(gates.values()) else "completed_not_promoted"
        ),
        ...
        "held_out_cases_loaded": len(cases),
        "held_out_cases_exposed_to_model": sum(
        ...
        "eligible_for_adjudication": all(gates.values()),
    ...
    print(
        f"passed={artifact['eligible_for_adjudication']} "
