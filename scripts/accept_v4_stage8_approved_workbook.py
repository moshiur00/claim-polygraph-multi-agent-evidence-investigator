"""Validate and preserve the human-approved V4.8 calibration workbook."""
...
SEED = ROOT / "benchmarks/verification_construction_v4_stage8_fresh_calibration_workbook_v1.json"
DESTINATION = (
    ROOT
    / "benchmarks/verification_construction_v4_stage8_fresh_calibration_workbook_v1_APPROVED.json"
)
AUDIT = ROOT / "artifacts/evaluations/verification-construction-v4-stage8-review-gate-v1.json"
...
def main() -> None:
    ...
    seed = load_replacement_calibration_workbook(SEED)
    submitted = load_replacement_calibration_workbook(args.source)
    if len(submitted.cases) != 20:
        raise ValueError("V4.8 requires exactly 20 cases")
    ...
    for case in submitted.cases:
        ...
        if claim_span is not None and (
            case.claim_text[claim_span.start_char : claim_span.end_char] != claim_span.quoted_text
        ):
        ...
        for span in annotation.evidence_spans:
            ...
            if (
                evidence is None
                or evidence.passage[span.start_char : span.end_char] != span.quoted_text
            ):
                raise ValueError(f"{case.case_id}: evidence span is not exact")
        label = annotation.gold_label.value
        label_counts[label] = label_counts.get(label, 0) + 1
        dimension = annotation.dimension_bucket
    ...
    audit = {
        "audit_id": "verification-construction-v4-stage8-review-gate-v1",
        "status": "approved_awaiting_v4_9_freeze",
        ...
        "next_action": "Freeze V4.9 calibration configuration and execute calibration exactly once",
    }
