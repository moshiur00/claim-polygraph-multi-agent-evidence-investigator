"""Freeze the approved V4.11 held-out boundary before its one allowed run."""
...
def main() -> None:
    if DESTINATION.exists():
        raise FileExistsError("V4.11 held-out boundary is already frozen")
    workbook = load_v4_fresh_held_out_workbook(WORKBOOK)
    if any(
        ...
        raise ValueError("V4.11 requires complete human annotation and distinct approval")
    review_gate = EVALUATIONS / "verification-construction-v4-stage10-review-gate-v1.json"
    ...
    if not review["review_gate_passed"] or review["approved_workbook_sha256"] != _hash(WORKBOOK):
        raise ValueError("approved workbook differs from the passed V4.10 review gate")
    calibration_path = (
        EVALUATIONS / "verification-construction-v4-stage9f-replacement-calibration-v1.json"
    )
    calibration = json.loads(calibration_path.read_text(encoding="utf-8"))
    if (
        calibration["calibration_executions"] != 1
        or not calibration["eligible_for_fresh_held_out_collection"]
        or not all(calibration["promotion_gates"].values())
    ):
        raise ValueError("V4.9f calibration did not authorize held-out evaluation")
    ...
    frozen_files = (
        ...
        EVALUATIONS / "verification-construction-v4-stage9d-temporal-remediation-audit-v1.json",
    ...
    manifest = {
        "manifest_id": "verification-construction-v4-stage11-held-out-evaluation-freeze-v1",
        "status": "frozen",
        "execution_limit": 1,
        "dataset_split": "fresh_held_out",
        ...
        "budget": {
            ...
            "maximum_held_out_cost_usd": 0.50,
        ...
        "held_out_executions_before_freeze": 0,
        "held_out_cases_exposed_to_model_before_freeze": 0,
        "artifacts": [
    ...
    print("status=frozen cases=20 execution_limit=1 held_out_model_calls=0")
