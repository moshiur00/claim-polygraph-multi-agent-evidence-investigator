"""Run the one-call V4.6a fresh temporal canary."""
...
EXPERIMENT_ID = uuid5(NAMESPACE_URL, "claim-polygraph/v4.6a/synthetic-canary/v1")
...
async def main() -> None:
    ...
    manifest_path = (
        root / "artifacts/evaluations/verification-construction-v4-stage6a-canary-manifest-v1.json"
    )
    ...
    ledger = SQLitePaidOperationLedger(root / "data/v4-stage6a-paid-operations.db")
    if ledger.list_receipts(EXPERIMENT_ID):
        raise ValueError("V4.6a requires a fresh empty receipt identity")
    telemetry = TelemetryCollector(root / "data/v4-stage6a-telemetry.db")
    ...
    artifact = {
        "artifact_id": "verification-construction-v4-stage6a-canary-result-v1",
        "status": (
            "passed"
    ...
    destination = (
        root / "artifacts/evaluations/verification-construction-v4-stage6a-canary-result-v1.json"
    )
