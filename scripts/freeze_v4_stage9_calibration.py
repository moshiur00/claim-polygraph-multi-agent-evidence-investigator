"""Freeze the approved V4.9 calibration boundary before its one allowed run."""
...
DESTINATION = EVALUATIONS / "verification-construction-v4-stage9-calibration-freeze-v1.json"
...
def main() -> None:
    if DESTINATION.exists():
        raise FileExistsError("V4.9 calibration boundary is already frozen")
    ...
    if any(
        ...
        raise ValueError("V4.9 requires complete human annotation and distinct approval")
    review_gate = EVALUATIONS / "verification-construction-v4-stage8-review-gate-v1.json"
    ...
    if not review["review_gate_passed"] or review["approved_workbook_sha256"] != _hash(WORKBOOK):
        raise ValueError("approved workbook differs from the passed V4.8 review gate")
    ...
    manifest = {
        "manifest_id": "verification-construction-v4-stage9-calibration-freeze-v1",
        "status": "frozen",
        "execution_limit": 1,
