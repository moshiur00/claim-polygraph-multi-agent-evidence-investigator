"""Run the frozen, receipt-protected V4.11 held-out evaluation exactly once."""
...
EXPERIMENT_ID = uuid5(NAMESPACE_URL, "claim-polygraph/v4.11/held-out-evaluation/v1")
MODEL = "gpt-4o-mini"
MAXIMUM_CALLS = 20
CALLABLE = {
...
async def main() -> None:
    ...
    if destination.exists():
        raise FileExistsError("V4.11 held-out evaluation has already been executed")

    freeze_path = (
        evaluations / "verification-construction-v4-stage11-held-out-evaluation-freeze-v1.json"
    )
    freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
    if freeze["status"] != "frozen" or freeze["execution_limit"] != 1:
        raise ValueError("V4.11 is not protected by a one-execution freeze")
    if freeze["budget"]["maximum_held_out_calls"] != MAXIMUM_CALLS:
        raise ValueError("frozen held-out call budget is not 20")
    ...
    workbook = load_v4_fresh_held_out_workbook(dataset_path)
    ...
    if ledger.list_receipts(EXPERIMENT_ID):
        raise RuntimeError("V4.11 receipt identity is not empty; refusing a rerun")
    telemetry = TelemetryCollector(root / "data/v4-stage11-telemetry.db")
    ...
    for case in cases:
        ...
        if eligibility not in CALLABLE:
            ...
            continue
        claim_id = uuid5(NAMESPACE_URL, f"claim-polygraph/v4.11/{case.case_id}")
        evidence = tuple(
            Evidence(
                evidence_id=uuid5(
                    NAMESPACE_URL,
                    f"claim-polygraph/v4.11/{case.case_id}/{item.evidence_id}",
                ),
                claim_id=claim_id,
        ...
        wrapped = IdempotentStructuredModelProvider(
            ...
            worker_id="v4.11-held-out-evaluation",
        )
    ...
    if cost.attempted_operation_count > MAXIMUM_CALLS:
        raise AssertionError("held-out call ceiling exceeded")
    ...
    artifact = {
        "evaluation_id": "verification-construction-v4-stage11-held-out-evaluation-v1",
        "status": (
            "completed_eligible_for_v4_promotion"
            if eligible
            else "completed_not_eligible_for_v4_promotion"
        ),
        ...
        "held_out_call_limit": MAXIMUM_CALLS,
        "held_out_calls_remaining": MAXIMUM_CALLS - cost.attempted_operation_count,
        "held_out_cases_loaded": len(results),
        "dataset_sha256": _sha256(dataset_path),
        "promotion_gates": gates,
        "eligible_for_v4_promotion": eligible,
