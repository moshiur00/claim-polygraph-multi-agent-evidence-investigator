"""Freeze the approved V3.6c fresh calibration before its one allowed run."""
...
def main() -> None:
    ...
    if any(
        ...
        raise ValueError("fresh calibration lacks complete distinct approval")
    ...
    manifest = {
        "manifest_id": (
            "verification-construction-v3-stage6c-fresh-calibration-freeze-v1"
        ),
        "status": "frozen",
        "execution_limit": 1,
        ...
        "fresh_workbook_sha256": _hash(workbook_path),
    ...
    if destination.exists():
        raise FileExistsError("V3.6c fresh freeze already exists")
