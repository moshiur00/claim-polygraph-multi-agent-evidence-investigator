"""Audit V4.9b fresh calibration collection before human review."""
...
def main() -> None:
    ...
    workbook_path = (
        root / "benchmarks/verification_construction_v4_stage9b_fresh_calibration_workbook_v1.json"
    )
    workbook = load_replacement_calibration_workbook(workbook_path)
    retired_claims: set[str] = set()
    retired_domains: set[str] = set()
    for pattern in (
        "benchmarks/verification_construction_v3*.json",
        "benchmarks/verification_construction_v4*.json",
    ):
        for path in root.glob(pattern):
            if path == workbook_path:
                continue
            payload = json.loads(path.read_text(encoding="utf-8"))
            _walk_records(payload, retired_claims, retired_domains)

    current_claims = {case.claim_text.casefold().strip() for case in workbook.cases}
    current_domains = {
        urlparse(evidence.url).netloc.casefold().removeprefix("www.")
        for case in workbook.cases
        for evidence in case.evidence
    }
    labels = {
        label: sum(case.proposal.suggested_gold_label.value == label for case in workbook.cases)
        for label in (
    ...
    for case in workbook.cases:
        try:
            ...
            for span in case.proposal.evidence_spans:
                if (
                    evidence[span.evidence_id].passage[span.start_char : span.end_char]
                    != span.quoted_text
                ):
    ...
    gates = {
        ...
        "calibration_only": all(case.split.value == "calibration" for case in workbook.cases),
        "exact_span_failures_zero": exact_span_failures == 0,
        "human_annotations_zero_before_review": all(
        ...
        "pdf_sources_zero": all(
            not evidence.url.casefold().endswith(".pdf")
            for case in workbook.cases
            for evidence in case.evidence
        ),
        "calibration_execution_zero": True,
        "held_out_cases_loaded_zero": True,
    }
    audit = {
        "audit_id": "verification-construction-v4-stage9b-collection-audit-v1",
        ...
        "gates": gates,
        "next_action": (
            "Human annotation by Md Moshiur Rahman and distinct approval by Md Rashedul Islam"
        ),
    }
    destination = (
        root / "artifacts/evaluations/verification-construction-v4-stage9b-collection-audit-v1.json"
    )
    destination.write_text(json.dumps(audit, indent=2) + "\n", encoding="utf-8")
    print(destination.relative_to(root))
    print(
        f"status={audit['status']} cases={audit['case_count']} "
        f"families={audit['origin_family_count']} model_calls=0"
    )
