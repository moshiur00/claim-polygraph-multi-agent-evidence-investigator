"""Freeze the approved V4.9f calibration boundary before its one allowed run."""
...
WORKBOOK = (
    ROOT
    / "benchmarks/verification_construction_v4_stage9e_fresh_calibration_workbook_v1_APPROVED.json"
)
DESTINATION = (
...
def main() -> None:
    if DESTINATION.exists():
        raise FileExistsError("V4.9f calibration boundary is already frozen")
    ...
    if any(
        ...
        raise ValueError("V4.9f requires complete human annotation and distinct approval")
    review_gate = EVALUATIONS / "verification-construction-v4-stage9e-review-gate-v1.json"
    ...
    if not review["review_gate_passed"] or review["approved_workbook_sha256"] != _hash(WORKBOOK):
        raise ValueError("approved workbook differs from the passed V4.9e review gate")
    ...
    manifest = {
        "manifest_id": "verification-construction-v4-stage9f-replacement-calibration-freeze-v1",
        "status": "frozen",
        "execution_limit": 1,
